...
```{r}
df <- data.frame(seed = 1:1000, random_number = NA_integer_)
for(i in 1:nrow(df)) {
	set.seed(df[i,]$seed)
	df[i,]$random_number <- sample(100, 1)
}
```

.pull-left[
```{r}
ggplot(df, aes(x = seed, y = random_number)) + geom_point()
```
]

.pull-right[
```{r}
cor.test(df$seed, df$random_number)
```
]
